...
- `runtime/opportunity/research_ai_input.py`：校验 Runtime Knowledge Snapshot，并把 Canonical 原文嵌入 AI Input；
...
- `runtime/opportunity/path_research_worker.py`：单任务独立子进程；
- `runtime/opportunity/path_research_batch.py`：受控小批次消费 persistent pending queue；
- `runtime/ai_runtime/prompts/path_research_v1.md`；
- `PATH_RESEARCH` 已注册到既有 AI Runtime；
- `path_evidence_search / path_evidence_fetch` 固定当前 task 的 stock_code / cutoff；
- AI Runtime validator 按 task spec 动态分发，原 Market Map AI 流程不受影响。

当前 V1 执行预算：

```text
max_tool_rounds = 4
wall_timeout = 180 seconds / task
default_auto_retry = false
```

每个 task 由独立 worker subprocess 运行。单只超时或失败不会拖死整个 batch；超时后 Trigger State 恢复 PENDING，pending task 保留，不写伪 Path Result。
...
真实 DeepSeek Runtime 已验证：

- 洽洽转债 / MATURITY_CASH：PASS / COMPLETED / review_ready=true；
- 仙乐转债 / DOWNWARD_REVISION：PASS / UNRESOLVED / review_ready=false；
- 乐歌转债 / MATURITY_CASH：PASS / COMPLETED；
- 天能转债 / MATURITY_CASH：PASS / COMPLETED；
- 冀东转债 / MATURITY_CASH：PASS / COMPLETED；
- 海顺转债 / DOWNWARD_REVISION：4轮工具内 PASS / COMPLETED / review_ready=true；
- 易瑞转债 / DOWNWARD_REVISION：0轮额外工具，PASS / NEEDS_EVIDENCE / review_ready=false。
...
Runtime PASS
+
Research Status = NEEDS_EVIDENCE / UNRESOLVED
+
review_ready = false
```

这表示程序和 validator 正常完成，但现实证据尚不足，不触发自动重试。

当前：

- Research Ledger = 10；
- persistent pending queue = 28；
- residual IN_PROGRESS = 0。

## 下一工程节点

当前仍处于 **Path Research Runtime / 批量执行阶段**。

当前 persistent pending queue = **28**。
...
- 180秒硬上限；
- 默认不自动重试；
- NEEDS_EVIDENCE / UNRESOLVED 正常写 Ledger，不视为 Runtime 失败。
